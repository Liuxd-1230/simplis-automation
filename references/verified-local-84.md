...
- `scripts/schematic_generator.py` generated and netlist-checked the hybrid-routing feedback divider example from `references\generated_feedback_divider_hybrid.json`; the generated script used short local `Wire /loc` segments plus one `term` label for `VOUT` and one for `VFB`. The generated netlist contained `.node_map VOUT`, `.node_map VFB`, `V1 5 0 12`, `R1 5 6 100k`, `R2 6 0 20k`, and `C1 6 0 10p`.
